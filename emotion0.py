# pipe曼荼羅：ライブ分割＆既知語増殖＋意味ある1字語を残しノイズ一字連鎖だけ自動枝刈り＋Blammo全抹消仕様

- context.txt から既知語リストを1行ずつ読み込む
- 入力文を既知語で最大長一致分割
- 既知語も他の既知語で最大長一致分割
- 新語・分割語が出ればcontext.txtに1行ずつ自動追加
- ただし、ツリーの3階層以下で“すべて1文字のノイズ的な連鎖”になっている枝（例：|カ→|レ→|ー→|う→|ど→|んなど）は自動剪定してcontext.txtから削除
- そして、もし既知語リストが“全て一字語のみ”になったら「Blammooo!」と叫んでcontext.txtを完全に抹消・初期化する
